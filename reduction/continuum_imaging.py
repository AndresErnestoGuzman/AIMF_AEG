...
for continuum_ms in continuum_mses:
    ...
    if exclude_7m:
        ...
        if os.getenv("USE_SELFCAL_MS"):
            ...
            # faster
            new_continuum_ms = continuum_ms.replace(".cal.cont", "_12M.cal.cont")
        ...
        # faster
        new_continuum_ms = continuum_ms.replace(".cal.cont", "_7M.cal.cont")
        split(vis=continuum_ms, outputvis=new_continuum_ms, antenna=antennae,
              field=field, datacolumn='data')
        continuum_ms = new_continuum_ms
    else:
    ...
    if chequear:
        chequear = False
        print "\nEnter to continue (Ctrl-C to kill)."
        r = raw_input()
